...
df2 = df[df['CUSTOMERID'] == cliente][['XMLFILEID','DATE','TOTALBEFORTAX',
...
with tab1:
    ...
    st.write("DESPLEGAMOS LAS ÚLTIMAS",df2.tail().count()['DATE'], " FACTURAS REGISTRADAS:")
...
with tab2:
    ...
    with col1:
        ...
        if opr1 == "Suma":
            gra1 = df2.groupby('MES').sum()
        elif opr1 == "Promedio":
            gra1 = df2.groupby('MES').mean()
        else:
            gra1 = df2.groupby('MES').count()
        ...
        gra3 = df2.groupby('PAYMETHODCODE').count()
...
with tab3:
    ...
    if df2['AÑO'].min() == df2['AÑO'].max():
        anio = st.slider("AÑO",(df2['AÑO'].min()-1),df2['AÑO'].max())
    else:
        anio = st.slider("AÑO",df2['AÑO'].min(),df2['AÑO'].max())
    dummy = df2[df2['AÑO'] == anio]
